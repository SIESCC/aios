...
async function main() {
    ...
    for (const rel of ecosystemRelations) {
        try {
            let source, target;
            if (rel.sourceType === 'startup')
                source = await prisma.aiStartup.findFirst({ where: { name: rel.sourceName } });
            if (rel.sourceType === 'tool')
                source = await prisma.aiTool.findFirst({ where: { name: rel.sourceName } });
            if (rel.targetType === 'tool')
                target = await prisma.aiTool.findFirst({ where: { name: rel.targetName } });
            if (rel.targetType === 'model')
                target = await prisma.aiModel.findFirst({ where: { name: rel.targetName } });
            if (source && target) {
    ...
    const submissions = [
        { type: 'TOOL_REVIEW', status: 'APPROVED', title: 'ChatGPT is a game changer', content: 'I have been using ChatGPT for 6 months and it has completely transformed my workflow. The GPT-4 model is incredibly capable at coding, writing, and analysis.', authorName: 'Alex Chen' },
        { type: 'PROMPT', status: 'APPROVED', title: 'SEO Blog Post Generator', content: 'Create a comprehensive SEO-optimized blog post about {TOPIC}. Include: title, meta description, H2 headers, key points, and a call to action. Target keyword: {KEYWORD}. Word count: 1500+.', authorName: 'Sarah K.' },
        { type: 'COMPARISON', status: 'APPROVED', title: 'Claude vs ChatGPT for Coding', content: 'After extensive testing, Claude 3.5 Sonnet edges out GPT-4o in coding tasks. Claude better understands complex codebases, while ChatGPT excels at quick scripts and boilerplate.', authorName: 'Dev Team' },
    ];
